                                ...
                                <div class="buttons-container">
                                    {if $slide.info->available_for_order && !isset($restricted_country_mode) && !$PS_CATALOG_MODE && $settings.cart_button}
                                        {if (!isset($slide.info->customization_required) || !$slide.info->customization_required) && $slide.info->quantity > 0}
                                            <a class="ajax_add_to_cart_button btn btn-md btn-default cart-button" href="{$link->getPageLink('cart', true, NULL, $smarty.capture.default, false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='tmproductsslider'}" data-id-product="{$slide.info->id|intval}" data-minimal_quantity="{$slide.info->minimal_quantity|intval}">
