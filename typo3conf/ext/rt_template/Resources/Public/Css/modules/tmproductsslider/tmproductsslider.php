...
class TMProductsSlider extends Module
{
        ...
        if (Tools::getIsset('slide_status')) {
            if ($id_slide = Tools::getValue('id_slide')) {
                if ($this->changeStatus($id_slide)) {
                    $output .= $this->displayConfirmation($this->l('Slide status successfully update'));
                } else {
                    $output .= $this->displayError($this->l('Some problem occurred during slide status changing.'));
                }
            } else {
    ...
    protected function getConfigForm()
    {
        ...
        $width_fields = $this->addPropertyField(
            ...
            array(/*name*/ 'slider_width', /*'lable'*/ 'Slider Width', /*'desc'*/ 'Slider Width',  /*'class'*/ '')
        );
        ...
        $height_fields = $this->addPropertyField(
            'text',
            array('standard', 'list', 'grid', 'fullwidth'),
            array(/*name*/ 'slider_height', /*'lable'*/ 'Height Width', /*'desc'*/ 'Height Width',  /*'class'*/ '')
        );
        ...
        $slider_interval = $this->addPropertyField(
            'text',
            array('standard', 'list', 'grid', 'fullwidth'),
            array(/*name*/ 'slider_interval', /*'lable'*/ 'Slider Interval', /*'desc'*/ 'Interval between slides showing in milliseconds.', /*'class'*/ 'extended autoplay')
        );
